...
#define NO_THREAD_PER_BLOCK 16 // Maximum value depends on the hardware used
#define MEM_SIZE 16384 //Can be set uto 2^17
#define ITER_GEMM 1
#define ITER_FFT 1
#define NO_INPUTS 128 // NO_INPUTS*CHANNELNO has to be less than 4096
#define NO_SAMPLES_PER_CHANNEL 16384//Maximum can be 16384 since 8192 is the maximum an image buffer can store and half of this value is stored in image buffer
#define CHANSIZE NO_INPUTS //Number of inputs per channel // Change this in kernel code named as the variable NO_INPUTS
#define CHANNELNO MEM_SIZE/NO_SAMPLES_PER_CHANNEL //Numbers of channel
...
int main()
{
	...
	size_t batchSize=NO_INPUTS;//number of discreet fft's to be calculated simultaneously
...
//===================================================================
	...
	//====================Finish the reshuffling================================//
        ...
        float** temp_reshuffle=sgenerate2darray((NO_INPUTS*CHANNELNO),NO_SAMPLES_PER_CHANNEL);

        for(int i=0;i<CHANNELNO;i++){
            for(int j=0;j<NO_INPUTS;j++){
                for(int k=0;k<NO_SAMPLES_PER_CHANNEL;k++){
                    temp_reshuffle[j+(i*NO_INPUTS)][k]=answer[j][k+(i*NO_SAMPLES_PER_CHANNEL)];
                }
            }
        }

        float** answer_final=screate2darray(NO_INPUTS*CHANNELNO*2,NO_SAMPLES_PER_CHANNEL/2);
        for(int i=0;i<NO_INPUTS*CHANNELNO;i++){
            for(int j=0;j<NO_SAMPLES_PER_CHANNEL;j++){
                if(j&1)
                        answer_final[(i<<1)+1][j >> 1]=temp_reshuffle[i][j];
                else
                        answer_final[(i<<1)][j >> 1]=temp_reshuffle[i][j];
            }
        }


	if(showReformatOutputAfterReshuffle){
        	for(int i=0;i<(NO_INPUTS*CHANNELNO)*2;i++){
        	    for(int j=0;j<NO_SAMPLES_PER_CHANNEL/2;j++){
	...
	OPENCL_V_THROW(clReleaseMemObject(clMemBuffersOut),"Error in releasing clMembufferout");
        ...
        float** corr_h=sgenerate2darrayout(NO_INPUTS*CHANNELNO,CHANSIZE << 1,CHANNELNO);// To be used to store the final answer
	...
	sizeofgpumem=((NO_INPUTS*CHANNELNO) << 1)*(NO_SAMPLES_PER_CHANNEL >> 1)*sizeof(float);
	cl_mem clMemBuffersIn_image=clCreateImage2D(context,CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,&imageFormat,(NO_SAMPLES_PER_CHANNEL >> 3),((NO_INPUTS*CHANNELNO) << 1),0,answer_final[0],&ret);
	OPENCL_V_THROW( ret, "Creating clMemBuffersIn Buffer failed" );

	if(showGemmInput){
                printf("\n");
                float** chk_inp1=screate2darray(((NO_INPUTS*CHANNELNO)<< 1),(NO_SAMPLES_PER_CHANNEL >> 1));
		size_t origin[] = {0, 0, 0};
	        size_t region[] = {(NO_SAMPLES_PER_CHANNEL >> 3), ((NO_INPUTS*CHANNELNO) << 1), 1};
        	OPENCL_V_THROW(clEnqueueReadImage(queue,clMemBuffersIn_image,1,origin,region,0,0,chk_inp1[0],0,0,0),"Reading image error 1");
		for(int i=0;i<((NO_INPUTS*CHANNELNO) << 1);i++){
			for(int j=0;j<(NO_SAMPLES_PER_CHANNEL >> 1);j++){
				cout << chk_inp1[i][j] << " ";
			}cout << endl;
		}
		cout << endl;
	...
	cl_mem clMemBuffersOut_image=clCreateImage2D(context,CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR,&imageFormat,(CHANSIZE >> 1),((NO_INPUTS*CHANNELNO)),0,corr_h[0],&ret);
	OPENCL_V_THROW (ret, "Creating output Buffer failed");


	int M1=CHANNELNO*((((NO_INPUTS*CHANNELNO)<<1)/(CHANNELNO*8))*((((NO_INPUTS*CHANNELNO)<<1)/(CHANNELNO*8))+1))/2;
	...
	int temp_NO_SAMPLES_PER_CHANNEL=NO_SAMPLES_PER_CHANNEL >> 1;
	int temp_imputs_size=((NO_INPUTS*CHANNELNO) << 1)/CHANNELNO;
	...
	OPENCL_V_THROW(clSetKernelArg(clKernel, 2, sizeof(cl_int), (void *)&temp_NO_SAMPLES_PER_CHANNEL),"Setting Arg 3 of matrix multiply failed");
	...
	size_t region_out_image[] = {(CHANSIZE >> 1), ((NO_INPUTS*CHANNELNO)), 1};
	...
	printf("Time for reshuffle: %lf\n",time_reshuffle_end-time_reshuffle_start);
        ...
        //**********To-do*********//
        //long long nflops=(ITER_GEMM*(CHANNELNO*(CHANSIZE*CHANSIZE*MEM_SIZE)));
        //printf("GFlops for matrix multiplication: %lf\n",(nflops/(1000000000*(t2_gemm-t1_gemm))));
